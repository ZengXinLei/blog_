  ...
  <div class="app-container">
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="150px" label="所在帖子">
        <template slot-scope="scope">
          <a v-if="scope.row.article!==null" :href="settings.url+'detail/'+scope.row.article.aId">{{ scope.row.article.aTitle.length>=10?scope.row.article.aTitle.substring(0,9)+"....":scope.row.article.aTitle }}</a>
          <a v-else :href="mySettings.url+'contact'" target="view">留言墙</a>
        </template>
      </el-table-column>
