  ...
  <ul>
    ...
    <li v-for="(comment,$key) in commentList">
      <div class="comment-detail">
        <img class="comment-detail-img" :src="require('@/assets/image/head/'+comment.tourist.tPhoto)">
        <div class="comment-detail-body">
          <div class="comment-detail-body-top">
            <span class="detail-top-nickname">
              {{comment.tourist.tName}} &nbsp;
<!--              <span v-if="comment.cReviewId===''" class="bz-icon">博主</span>-->
            </span>
<!--            <span class="detail-top-area">&nbsp;&nbsp;{{comment.cText}} &nbsp;</span>-->
            <span style="float:right;color:#aaa;">{{ $key+1}}楼 &nbsp;{{dealDate(comment.cTime)}}</span>
          </div>
          <div v-if="comment.comment!==null" class="comment-detail-response">
            <h5>引用 {{comment.comment.tourist.tName}}
              <font style="font-size: 12px;float: right;color:#aaa;font-weight: 400;">发表时间：{{dealDate(comment.comment.cTime)}}</font>
<!--              引用的内容-->
            </h5>
            {{comment.comment.cText}}
          </div>
          <div v-html="comment.cText" class="comment-detail-body-bottom"></div>
      ...
      <div v-if="comment.isResponse">
        ...
        v-on:submit-comment="submitCommentResponse($event,comment.cId,$key)"
...
export default {
  name: 'commentItem',
  props: ['commentList','tourist','article'],
  components: {
    LoadingSpining,
  ...
  methods: {
    ...
    submitCommentResponse: function (data, id, index) {
      ...
      this.$emit("addComment",data, id, index)
