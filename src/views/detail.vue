...
export default {
  ...
  created () {
    ...
    let aId = this.$route.params.articleId;
    getArticleById(aId).then(res=>{
    ...
    //   readArticle({
    //     lArticleId:aId,
    //     lTouristId:tourist.tId,
    //     lTime:new Date().getTime()/1000>>0
    //   })
    // })
  ...
  methods: {
    ...
    getArticleDetail () {
      ...
      this.title = article.aTitle;
      this.viewBackstyle.backgroundImage = `url(${require('../' + article.photoUrl)})`;
      this.tagsArr = article.aType.split('|');
      this.author = article.user.uName;
      this.time = article.aTime;
      this.readCount = article.aReadCount;
      this.commentCount = article.aReviewCount;
    },
    getLifeDetail () {
